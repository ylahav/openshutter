...
### Shipped on this branch (PWA)


| Change                              | Files                                                                                               |
| ----------------------------------- | --------------------------------------------------------------------------------------------------- |
| Album grid lazy-load + small thumbs | `AlbumGridImage.svelte`, `getPhotoGridUrl()` in `photoUrl.ts`, templates + `PhotoCard`              |
| Progressive lightbox + prefetch     | `PhotoLightbox.svelte`, `lightboxImageCache.ts`                                                     |
| PWA manifest, SW, install banner    | `manifest.json`, `sw.js`, `PwaInstallBanner.svelte`, `pwa-install.ts`, `+layout.svelte`, `app.html` |


### Shipped earlier (PWA performance)
...
- **Baseline metrics:** re-run `node scripts/album-image-trace.mjs` after grid deploy; compare small vs medium tier bytes.
...
- **Document tiers (v1):** grid = `getPhotoUrl` (medium/small); lightbox = preview via `resolveLightboxUrls` then full; prefetched neighbors use browser image cache.
...
- `**PhotoLightbox` progressive load** — `resolveLightboxUrls` + preview first, then full; small “HD…” hint while upgrading (`lightboxImageCache.ts`).
- **Adjacent prefetch** — prev/next (+1 ahead) preload preview immediately, full in `requestIdleCallback`.
...
- **Installable PWA (v1):** assets in `**frontend/static/`** (SvelteKit serves these at `/` — not `public/`), `manifest.json`, `app.html` meta, icons (`pnpm pwa:icons`), `sw.js`, `PwaInstallBanner` + service worker on public routes.
- Improve **lightbox** load on mobile (progressive + prefetch — see Shipped).
