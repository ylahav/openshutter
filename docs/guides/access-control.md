...
- **Admin photo APIs** (`/api/admin/photos/:id`, PUT, DELETE, regenerate-thumbnails, rotate, re-extract-exif; bulk/re-extract-exif, bulk/regenerate-thumbnails, bulk/regenerate-thumbnails-stream): Owners can call these; backend verifies the photo’s album was created by the owner before allowing access.
