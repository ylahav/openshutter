# White-label (design & deployment)

## White-Label Solutions Design

**Phase:** 4 – Stage 1  
**Status:** **Complete** (March 2026) — Solution 1 (single-install branding) and Solution 2 (per-owner domains, scoped public APIs, owner admin). Optional follow-ups (sitemaps, primary canonical, storage model polish, schema extras in §2.1 vs code) are **low priority** and tracked in [`PHASE_4_WORKFLOW.md`](../archive/development/PHASE_4_WORKFLOW.md) §1.2.6–1.2.7.  
...
5. **Admin APIs for owner domains and storage**
   ...
   - **Status:** Implemented admin CRUD for owner domains and Owner Domains section in the Users admin page. **Storage:** global `storage_configs` + per-owner **`storageConfig`** on **`/owner/storage`**; **dedicated** path via **`useDedicatedStorage`** and **`owner_storage_configs`** (see [`PHASE_4_WORKFLOW.md`](../archive/development/PHASE_4_WORKFLOW.md) §1.2.6). Optional UX gap: configure dedicated storage in the same flow as **creating** an owner.
...
- [x] Runbook: **deployment section** in this document (canonical URLs, env, owner domains, TLS pointers).
...
- [PHASE_4_WORKFLOW.md](../archive/development/PHASE_4_WORKFLOW.md) — White-label solutions (Section 1)
...
3. **Register the hostname**  
   In **Admin → Users**, edit the **owner** user → **Owner Domains** → add the exact hostname the browser sends (including **port** in local dev, e.g. `sara.localhost:4000`). See [Testing owner domains on localhost](#testing-owner-domains-on-localhost) above.
...
5. **Content**  
   Public albums, photos, search, CMS pages, and blog list/article APIs are **scoped by owner** when the request host maps to an owner domain (see **[`PHASE_4_WORKFLOW.md`](../archive/development/PHASE_4_WORKFLOW.md)** §1.2).
