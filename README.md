...
3. **Set up environment variables**
   ...
   **Backend** (`backend/.env`):
   ...
   # Optional: Separate URL for email links (falls back to FRONTEND_URL)
   EMAIL_BASE_URL=http://localhost:4000
   # Optional: Separate URL for Google Drive OAuth callback (falls back to FRONTEND_URL)
   GOOGLE_OAUTH_CALLBACK_BASE_URL=http://localhost:4000
   ```
...
2. **Deploy to server**
   ...
   # On server: Extract in place, install dependencies, and start with PM2
...
- [x] **Face detection – remove a stored rectangle** – Select a face on the overlay, then **Remove face** or **Delete** / **Backspace**; **`POST /api/admin/face-recognition/remove-face`**
- [x] **Photo editing capabilities** (rotate, crop)
  - [x] **Display rotation** (90° CW, 90° CCW, 180°) from the photo edit page (admin and owner): **`POST /api/admin/photos/:id/rotate`** with **`{ angle: 90 | -90 | 180 }`** **accumulates** with the stored **`rotation`** (normalized; unset when net rotation is **0**). **`canRestoreOriginal`** is true when a pre-crop file backup exists **or** when display rotation is non-zero; **`POST /api/admin/photos/:id/restore-original`** clears rotation (rotation-only undo) or restores the file backup and clears rotation.
  - [x] Crop single photo from the photo edit page (admin and owner) via **`PhotoCropper.svelte`** (responsive canvas, pan when zoomed). Backend replaces the file, regenerates thumbnails, re-extracts EXIF, snapshots the pre-crop file as **`originalBackupPath`** for restore, and translates stored **`faceRecognition.faces[].box`** / **`landmarks`** into the new image coordinate system (faces fully outside the crop are dropped; partial overlaps are clipped). API: **`POST /api/admin/photos/:id/crop`** with **`{ x, y, width, height }`** in original-image pixels; restore via **`POST /api/admin/photos/:id/restore-original`**.
  - [x] **Photo edit feedback** – Success and error messages use a bottom **`Toast`** (about **5** seconds, non-blocking) instead of a modal dialog; no automatic scroll to the face-matching section after face reload.
