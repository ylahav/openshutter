...
export { AuthenticationError, BackendHttpError };
 ...
 * 
 * Throws AuthenticationError for 401, BackendHttpError for other non-OK responses.
 */
export async function parseBackendResponse<T>(response: Response): Promise<T> {
	if (!response.ok) {
		...
		throw new BackendHttpError(message, response.status, errorData);
	}
