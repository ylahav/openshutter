<script lang="ts">
  ...
  export let layoutPresetsPreview: Record<string, unknown> | null | undefined =
    undefined;

  const layoutPresetsPreviewStore: Writable<Record<string, unknown> | null> =
    writable(null);
  ...
  $: routeParams = ($pageStore.params || {}) as Record<
    string,
    string | undefined
  >;
  $: routeAlias =
  ...
  $: normalizedModules = modules.map((m) => ({
    ...m,
    type: normalizeModuleType((m as any).type),
  }));
...
{#if !page}
  <div
    class="min-h-screen flex items-center justify-center text-(--tp-fg-muted)"
  >
    Page not found.
  </div>
{:else}
