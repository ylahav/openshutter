			...
			{#each users as user}
				<tr class="hover:bg-(--color-surface-50-950)">
					<td class="px-6 py-4 whitespace-nowrap">
						<div>
							<div class="text-sm font-medium text-(--color-surface-950-50)">{getUserName(user)}</div>
							<div class="text-sm text-(--color-surface-600-400)">{user.username}</div>
						</div>
					</td>
					<td class="px-6 py-4 whitespace-nowrap">
						<div class="flex flex-wrap items-center gap-1">
							<span
								class="px-2 py-1 text-xs font-medium rounded {user.role === 'admin'
									...
									: user.role === 'owner'
										? 'bg-[color-mix(in_oklab,var(--color-primary-500)_22%,transparent)] text-(--color-primary-800)'
										: 'bg-(--color-surface-100-900) text-(--color-surface-900-100)'}"
							>
					...
					<td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
						<button
							...
							class="text-(--color-primary-600) hover:text-(--color-primary-900) mr-4"
						>
							Edit
						</button>
						<button
