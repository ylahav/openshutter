...
export const POST: RequestHandler = async ({ request, locals, cookies }) => {
	try {
		...
		const response = await backendPost('/admin/face-recognition/bulk-detect', body, { cookies });
		...
		logger.error('Bulk face detection error:', error);
		const parsed = parseError(error);
		return json({ 
			success: false, 
			error: parsed.userMessage || 'Bulk face detection failed' 
