...
export const GET: RequestHandler = async ({ url, locals, cookies }) => {
	try {
		...
		return json({ success: false, error: errorMessage || 'Failed to fetch tags' }, { status: 500 });
	}
};
...
export const POST: RequestHandler = async ({ request, locals, cookies }) => {
	try {
		...
		const response = await backendPost('/admin/tags', body, { cookies });
		...
		const errorMessage = error instanceof Error ? error.message : String(error);
		return json({ success: false, error: errorMessage || 'Failed to create tag' }, { status: 500 });
	}
};
