	...
	try {
		...
		if (updates.location !== undefined) {
			if (updates.location && updates.location.name) {
				...
				if (location) {
					// Use Mongoose ObjectId directly
		...
		const mongoosePhotoIds = photoIds
			.map((id: string) => {
				try {
		...
		const result = await PhotoModel.updateMany({ _id: { $in: mongoosePhotoIds } }, { $set: updateData });
		...
		if (updates.isLeading !== undefined) {
			...
			for (const photo of photos) {
				...
				if (updates.isLeading) {
					// Set as cover
					await albumsCollection.updateOne({ _id: photo.albumId }, { $set: { coverPhotoId: photo._id } });
