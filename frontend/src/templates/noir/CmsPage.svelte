...
<div class="min-h-screen w-full bg-(--tp-canvas) text-(--tp-fg) [font-family:var(--os-font-body)] px-8 py-16">
	{#if error}
		<div class="max-w-md mx-auto text-center py-20">
			<p class="text-[9px] uppercase tracking-[0.28em] text-(--tp-fg-subtle) mb-4">error</p>
	...
	{:else if page}
		<article class="max-w-3xl mx-auto">
			<p class="text-[9px] uppercase tracking-[0.28em] text-(--tp-fg-subtle) mb-4">page</p>
			...
			{#if page.subtitle}
				<p class="text-[13px] text-(--tp-fg-muted) mb-10 leading-relaxed">
					{MultiLangUtils.getTextValue(page.subtitle, $currentLanguage)}
				</p>
			{/if}
			{#if page.introText}
				<div class="text-sm text-(--tp-fg-muted) mb-8 leading-relaxed">
					{@html MultiLangUtils.getHTMLValue(page.introText, $currentLanguage)}
				</div>
			{/if}
			{#if page.content}
				<div class="text-sm text-(--tp-fg) leading-relaxed space-y-4">
