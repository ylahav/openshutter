...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!photo.faceRecognition?.faces || photo.faceRecognition.faces.length === 0) {
      return NextResponse.json({ success: false, error: 'No faces detected in photo' }, { status: 400 })
    }
    ...
    await db.collection('photos').updateOne(
      { _id: new ObjectId(photoId) },
      {
        $set: {
          ...
          people: [...new Set([...(photo.people || []).map((p: any) => p.toString()), ...matchedPersonIds.map(id => id.toString())])].map(id => new ObjectId(id))
        }
      }
    )
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        matches: matches.map(m => ({
          ...
          personName: m.personId ? people.find(p => p._id.toString() === m.personId)?.fullName : null
        }))
      }
    })
  } catch (error) {
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : 'Face matching failed' },
      { status: 500 }
    )
  }
}
