...
### Changed
- **Shared UI components:** Site-facing chrome controls (`Menu`, `LanguageSelector`, `ThemeToggle`, `TemplateSelector`) live under **`frontend/src/lib/components/ui/<name>/`** with a **README per component**. Imports use **`$components/ui/...`**. **Admin →** dashboard includes **UI components** linking to **`/admin/docs/ui`**, which renders those README sources for staff.
