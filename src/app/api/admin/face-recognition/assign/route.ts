...
export async function POST(request: NextRequest) {
  try {
    ...
    if (faceIndex === undefined || faceIndex === null) {
      return NextResponse.json({ success: false, error: 'Face index is required' }, { status: 400 })
    }
    ...
    if (faceIndex < 0 || faceIndex >= photo.faceRecognition.faces.length) {
      return NextResponse.json({ success: false, error: 'Invalid face index' }, { status: 400 })
    }
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : 'Face assignment failed' },
      { status: 500 }
    )
  }
}
