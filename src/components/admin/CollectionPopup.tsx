...
export default function CollectionPopup({
  ...
  const fetchItems = async (query: string = '') => {
    try {
      ...
      if (result.success) {
        ...
        if (collectionType === 'locations' && items.length > 0) {
          console.log('CollectionPopup - Locations API response:', result)
          console.log('CollectionPopup - First location:', items[0])
          console.log('CollectionPopup - First location keys:', Object.keys(items[0]))
          console.log('CollectionPopup - First location name:', items[0].name)
          console.log('CollectionPopup - First location names:', (items[0] as any).names)
        }
        setItems(items)
      } else {
  ...
  const createItem = async () => {
    ...
    try {
      ...
      if (collectionType === 'tags') {
        ...
        // For people, we need both firstName and lastName
        // If the user enters a full name, try to split it, otherwise use it as both
        const trimmedName = newItemName.trim()
        const nameParts = trimmedName.split(/\s+/)
        let firstName = trimmedName
        let lastName = trimmedName
        
        // If there are multiple words, use first word as firstName, rest as lastName
        ...
        requestBody = {
          ...
          // fullName will be auto-generated by the Person model pre-save hook
        }
  ...
  const toggleItem = (item: any) => {
    ...
    if (collectionType === 'people') {
      ...
      const nameField = item.name || (item as any).names
      identifier = String(
        typeof nameField === 'string' 
          ? nameField 
          : MultiLangUtils.getTextValue(nameField || {}, currentLanguage) || ''
      )
    } else {
      identifier = String(item.name || '')
    }
  ...
  const filteredItems = items.filter(item => {
    ...
    if (collectionType === 'people') {
      ...
      // Also search in address fields for locations
