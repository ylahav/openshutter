...
export default function Header() {
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center">
              <div className="flex items-center space-x-3">
                <div className="w-8 h-8 bg-gray-700 rounded animate-pulse shrink-0"></div>
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      isScrolled 
        ? 'bg-gray-900/95 backdrop-blur-sm border-b border-gray-800 shadow-2xl shadow-black/20' 
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center">
            <Link href="/" className="flex items-center space-x-3 group">
              {config?.logo ? (
                <div className="relative w-8 h-8 rounded overflow-hidden ring-2 ring-gray-700 group-hover:ring-blue-500 transition-all duration-300">
                  <Image
                    src={config.logo}
                ...
                <div className="w-8 h-8 bg-gradient-to-br from-blue-500 to-purple-600 rounded flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
          ...
          <div className="flex items-center space-x-4">
            ...
            {isComponentVisible('authButtons') && isLoggedIn ? (
              <div className="flex items-center space-x-3">
                ...
                <button
                  ...
                  className="text-sm font-medium text-gray-300 hover:text-white transition-colors duration-300 px-3 py-1 rounded hover:bg-gray-800"
                >
                  {t('header.logout')}
                </button>
              </div>
                ...
                <Link
                  href="/login"
                  className="px-4 py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white text-sm font-medium rounded-lg hover:from-blue-500 hover:to-purple-500 transition-all duration-300 shadow-lg hover:shadow-blue-500/25 transform hover:scale-105"
                >
                  {t('auth.signIn')}
                </Link>
              ) : null}
            ...
            <button
              ...
              className="md:hidden p-2 text-gray-300 hover:text-white transition-colors duration-300 rounded-lg hover:bg-gray-800"
            >
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 border-t border-gray-800 bg-gray-900/95 backdrop-blur-sm">
              {navigation.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                ...
                {isLoggedIn ? (
                  <div className="space-y-2">
                    ...
                    <button
                      ...
                      className="w-full text-left px-3 py-2 text-base font-medium text-gray-300 hover:text-white hover:bg-gray-800 transition-colors duration-300 rounded-lg"
                    >
                      {t('header.logout')}
                    </button>
                  </div>
                  ...
                  <Link
                    href="/login"
                    className="block px-3 py-2 text-base font-medium text-white bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 transition-all duration-300 rounded-lg"
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
                    {t('auth.signIn')}
                  </Link>
