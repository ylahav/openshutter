...
export class DatabaseOptimizer {
  ...
  static async getBatchAlbumsWithPhotos(
    ...
    const objectIds = albumIds.map(id => new Types.ObjectId(id))
  ...
  static async searchPhotos(
    ...
    // Album filter
    if (filters.albumId) {
      matchStage.albumId = { $in: [new Types.ObjectId(filters.albumId), filters.albumId] }
    }
  ...
  static async getPaginatedResults<T>(
    ...
    if (cursor) {
      const cursorValue = sortField === '_id' ? new Types.ObjectId(cursor) : cursor
