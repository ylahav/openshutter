...
function isInitialAdmin(email, password) {
...
let AuthService = class AuthService {
    ...
    validateUser(email, password) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const existingAdmin = yield this.userModel.findOne({ role: 'admin' });
            ...
            const user = yield this.userModel.findOne({ username: email });
            if (!user) {
                return null;
            }
            ...
            const ok = yield verifyPassword(password, user.passwordHash);
            if (!ok) {
                return null;
            }
            return {
