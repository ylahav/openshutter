...
class ExifExtractor {
     ...
     */
    static parseExifDate(dateValue) {
        try {
            if (!dateValue)
                return null;
    ...
    static extractExifData(imageBuffer) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (result.tags.SerialNumber)
                    exifData.serialNumber = result.tags.SerialNumber;
                ...
                if (result.tags.DateTimeOriginal) {
                    ...
                    if (parsedDate)
                        exifData.dateTimeOriginal = parsedDate;
                }
                if (result.tags.DateTimeDigitized) {
                    ...
                    if (parsedDate)
                        exifData.dateTimeDigitized = parsedDate;
                }
                if (result.tags.OffsetTime)
                    exifData.offsetTime = result.tags.OffsetTime;
                if (result.tags.OffsetTimeOriginal)
                    exifData.offsetTimeOriginal = result.tags.OffsetTimeOriginal;
                if (result.tags.OffsetTimeDigitized)
                    exifData.offsetTimeDigitized = result.tags.OffsetTimeDigitized;
                // Camera Settings
                ...
                if (result.tags.ISO || result.tags.ISOSpeedRatings)
                    exifData.iso = result.tags.ISO || result.tags.ISOSpeedRatings;
                if (result.tags.FocalLength)
                    exifData.focalLength = result.tags.FocalLength;
                if (result.tags.ExposureProgram)
                    exifData.exposureProgram = result.tags.ExposureProgram;
                if (result.tags.ExposureMode)
                    exifData.exposureMode = result.tags.ExposureMode;
                if (result.tags.ExposureBiasValue)
                    exifData.exposureBiasValue = result.tags.ExposureBiasValue;
                if (result.tags.MaxApertureValue)
                    exifData.maxApertureValue = result.tags.MaxApertureValue;
                if (result.tags.ShutterSpeedValue)
                    exifData.shutterSpeedValue = result.tags.ShutterSpeedValue;
                if (result.tags.ApertureValue)
                    exifData.apertureValue = result.tags.ApertureValue;
                // Image Quality
                if (result.tags.WhiteBalance)
                    exifData.whiteBalance = result.tags.WhiteBalance;
                if (result.tags.MeteringMode)
                    exifData.meteringMode = result.tags.MeteringMode;
                if (result.tags.Flash)
                    exifData.flash = result.tags.Flash;
                if (result.tags.ColorSpace)
                    exifData.colorSpace = result.tags.ColorSpace;
                if (result.tags.CustomRendered)
                    exifData.customRendered = result.tags.CustomRendered;
                if (result.tags.SceneCaptureType)
                    exifData.sceneCaptureType = result.tags.SceneCaptureType;
                // Resolution
                if (result.tags.XResolution)
                    exifData.xResolution = result.tags.XResolution;
                if (result.tags.YResolution)
                    exifData.yResolution = result.tags.YResolution;
                if (result.tags.ResolutionUnit)
                    exifData.resolutionUnit = result.tags.ResolutionUnit;
                if (result.tags.FocalPlaneXResolution)
                    exifData.focalPlaneXResolution = result.tags.FocalPlaneXResolution;
                if (result.tags.FocalPlaneYResolution)
                    exifData.focalPlaneYResolution = result.tags.FocalPlaneYResolution;
                if (result.tags.FocalPlaneResolutionUnit)
                    exifData.focalPlaneResolutionUnit = result.tags.FocalPlaneResolutionUnit;
                // Lens Information
                if (result.tags.LensInfo)
                    exifData.lensInfo = result.tags.LensInfo;
                if (result.tags.LensModel)
                    exifData.lensModel = result.tags.LensModel;
                if (result.tags.LensSerialNumber)
                    exifData.lensSerialNumber = result.tags.LensSerialNumber;
                ...
                if (result.tags.ExifVersion)
                    exifData.exifVersion = result.tags.ExifVersion;
                // Additional Technical Data
                if (result.tags.RecommendedExposureIndex)
                    exifData.recommendedExposureIndex = result.tags.RecommendedExposureIndex;
                if (result.tags.SubsecTimeOriginal)
                    exifData.subsecTimeOriginal = result.tags.SubsecTimeOriginal;
                if (result.tags.SubsecTimeDigitized)
                    exifData.subsecTimeDigitized = result.tags.SubsecTimeDigitized;
                // GPS information
     ...
     */
    static extractAndUpdateExif(photo, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                console.log(`   Storage provider: ${(_a = photo.storage) === null || _a === void 0 ? void 0 : _a.provider}`);
                console.log(`   Storage path: ${(_b = photo.storage) === null || _b === void 0 ? void 0 : _b.path}`);
                // Get the storage provider
                const storageProvider = ((_c = photo.storage) === null || _c === void 0 ? void 0 : _c.provider) || 'local';
                ...
                try {
                    ...
                    const cfg = (_e = (_d = storageService).getConfig) === null || _e === void 0 ? void 0 : _e.call(_d);
                    if (cfg && cfg.basePath) {
                ...
                const fileBuffer = yield storageService.getFileBuffer(photo.storage.path);
                if (!fileBuffer) {
                    console.warn(`Could not download file for EXIF extraction: ${photo.storage.path}`);
                    return photo;
                }
                // Extract EXIF data
                const exifData = yield this.extractExifData(fileBuffer);
                // Update the photo in database
                if (exifData) {
                    yield Photo_1.PhotoModel.updateOne({ _id: photo._id }, { $set: { exif: exifData } });
                }
                // Return updated photo (preserve existing exif if new is null)
                const updatedPhoto = Object.assign(Object.assign({}, photo), { exif: (_f = exifData !== null && exifData !== void 0 ? exifData : photo.exif) !== null && _f !== void 0 ? _f : null });
                if (exifData) {
     ...
     */
    static processPhotosForExif(photos) {
        return __awaiter(this, void 0, void 0, function* () {
            const processedPhotos = [];
            for (const photo of photos) {
                try {
                    const processedPhoto = yield this.extractAndUpdateExif(photo);
...
exports.ExifExtractor = ExifExtractor;
