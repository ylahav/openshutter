...
class CacheManager {
     ...
     */
    static getCacheHeaders(contentType) {
    ...
    /**
     * Apply cache headers to an Express response
     */
    static applyCacheHeaders(response, contentType, customHeaders) {
     ...
     */
    static shouldServeFromCache(request, contentType) {
        const config = this.CACHE_CONFIGS[contentType] || this.CACHE_CONFIGS.api;
        if (config.noCache || config.noStore) {
            return false;
        }
     ...
     */
    static generateCacheKey(resourceType, identifier, params) {
